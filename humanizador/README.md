...
remove AI-writing tells (the em-dash used as an aside, inflated language, forced
"not only X but also Y" symmetry, empty emphasis, corporate jargon, emoji-laden
formatting) while preserving meaning and never adding facts. Operates on
Portuguese text; useful for the Brazilian market specifically. Triggers on
"humanizar / tirar cara de IA" and variations.

---

## O que faz

Pega um texto com cara de ChatGPT e devolve uma versão que soa como pessoa. A
filosofia é simples: **humanizar é cortar, não enfeitar.** Texto de IA tem
gordura: rodeios, ênfases vazias, simetrias forçadas, travessão por todo lado. A
skill remove isso e devolve o ritmo irregular da escrita humana. O resultado é
quase sempre mais curto que o original.
...
- **Proíbe o travessão (—).** Essa é a regra que mais pega gente desprevenida. O
  travessão usado como aparte é um dos sinais mais fortes de texto de IA, e uma
  skill de humanização que solta travessão se contradiz. Por isso a proibição
  está nas regras inegociáveis, o travessão é o primeiro item do catálogo de
  sinais, e a própria documentação da skill foi escrita sem travessão. A skill
  pratica o que prega.
- **Regras inegociáveis no topo do `SKILL.md`.** Preservar sentido, nunca
  inventar, não inflar, respeitar o registro, nada de travessão. Elas vêm antes
  do fluxo de propósito: o maior risco de uma skill de reescrita é "melhorar" o
  texto inventando coisas ou trocando o tom. As regras prendem isso.
- **O catálogo de sinais fica em `reference/`, não no corpo.** Divulgação
  progressiva: o `SKILL.md` lista os sinais mais comuns; o catálogo detalhado com
  exemplos só entra no contexto quando é preciso. Mantém a skill leve.
...
- **Suporte a inglês.** Duplique o catálogo de sinais para EN e amplie a
  `description`. Vira um humanizador bilíngue (o travessão, aliás, é tell em EN
  também).
...
declarado como gerado por IA. Use para escrever melhor, não para burlar regras de
transparência de onde você publica.
